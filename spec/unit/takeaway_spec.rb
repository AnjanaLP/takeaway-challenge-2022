...
describe Takeaway do
  ...
  let(:order)             { double :order, total: 11, add_to_basket: nil, basket: nil }
  let(:sms)               { double :sms }
  ...
  describe '#add' do
    ...
    it 'returns the order basket' do
      allow(order).to receive(:basket).and_return(salad: 3)
      expect(takeaway.add(salad: 3)).to eq(salad: 3)
    end
  end
